...
// FİYAT HESAPLAMA FONKSİYONU (Backend'de de kullanabilmek için)
function getGuncelFiyat($conn) {
    ...
    $dolu_slotlar = [];
    while($r = $rez_db->fetch_assoc()) { $dolu_slotlar[] = $r['slot_adi']; }
    
    $g_dolu = count(array_unique($dolu_slotlar));
    for($i=1; $i<=4; $i++) {
        if(isset($ayarlar["slot$i"]) && $ayarlar["slot$i"] == 1 && !in_array("slot$i", $dolu_slotlar)) {
            $g_dolu++;
        }
    }
    ...
    if($yuz >= 25 && $yuz < 50) $fiyat = $taban * 1.25;
    elseif($yuz >= 50 && $yuz < 75) $fiyat = $taban * 1.5;
    elseif($yuz >= 75) $fiyat = $taban * 2;
    
    return ['fiyat' => round($fiyat), 'yuzde' => $yuz];
}

$fiyat_bilgi = getGuncelFiyat($conn);
$g_saatlik = $fiyat_bilgi['fiyat'];
$y_doluluk = $fiyat_bilgi['yuzde'];

// ... [Diğer POST işlemleri (arac_ekle, rezervasyon_yap) aynı kalabilir] ...
// Not: Rezervasyon yap kısmında toplam_tutar hesaplanırken getGuncelFiyat()['fiyat'] kullanmalısın.
...
<head>
    ...
    <script>
        // ... [Aynı JS fonksiyonları] ...
        
        function hesapla() {
            let sure = parseInt(document.getElementById('sure_secim').value) || 1;
            let birimFiyat = <?= $g_saatlik ?>;
            document.getElementById('toplam_tutar').innerText = sure * birimFiyat;
        }
    </script>
</head>
<!-- ... [Geriye kalan HTML yapısı aynı] ... -->
...
<body class="bg-slate-50 min-h-screen">
    ...
    <div class="max-w-6xl mx-auto p-6 mt-8">
        ...
        <?php if($y_doluluk >= 25): ?>
            <div class="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4 mb-8 rounded shadow-sm flex items-center">
                <i class="fa-solid fa-chart-line text-2xl mr-4"></i>
                <div>
                    <p class="font-bold text-lg">Dinamik Fiyatlandırma Aktif (Otopark Doluluk: %<?= $y_doluluk ?>)</p>
                    <p>Mevcut yoğunluk sebebiyle saatlik park ücreti <?= $t_fiyat ?> TL yerine <strong><?= $g_saatlik ?> TL</strong> olarak güncellenmiştir.</p>
                </div>
            </div>
        <?php endif; ?>
    ...
    <div id="rezervasyon_modal" class="fixed inset-0 bg-black/70 hidden flex items-center justify-center z-50">
        <div class="bg-white p-8 rounded-2xl w-full max-w-md shadow-2xl relative">
            ...
            <div class="bg-indigo-50 border border-indigo-100 p-5 rounded-xl mb-6 shadow-sm">
                <div class="flex justify-between items-center mb-3">
                    <span class="font-bold text-indigo-900 text-sm uppercase">Otopark Doluluk</span>
                    <span class="font-black text-indigo-600">%<?= $y_doluluk ?></span>
                </div>
                <div class="flex justify-between items-center mb-3 border-b border-indigo-200 pb-3">
                    <span class="font-bold text-indigo-900 text-sm uppercase">Güncel Saatlik Ücret</span>
                    <div class="text-right">
                        <?php if($y_doluluk >= 25): ?>
                            <span class="text-xs text-red-400 line-through mr-2"><?= $t_fiyat ?> TL</span>
                        <?php endif; ?>
                        <span class="font-black text-indigo-600 text-lg"><?= $g_saatlik ?> TL</span>
                    </div>
                </div>
                <div class="flex justify-between items-center pt-2">
                    <span class="font-black text-indigo-900 text-lg uppercase">ÖDENECEK TUTAR</span>
                    <span class="font-black text-3xl text-green-600"><span id="toplam_tutar"><?= $g_saatlik ?></span> TL</span>
                </div>
            </div>
