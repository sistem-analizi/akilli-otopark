...
class SQLiteKoprusu {
    ...
    /**
     * DB ilk açılışta yaratılırken schema.sql'i exec eder. Şema ve seed verisi
     * baglanti.php yanındaki schema.sql dosyasında (MySQL dump'tan çevrildi).
     * Şemayı güncellemek için: schema.sql düzenle, sunucudaki .db dosyasını
     * sil → bir sonraki request yeniden seed eder.
     */
    private function veritabaniniInsaEt() {
        $schema_yolu = __DIR__ . '/schema.sql';
        if (!file_exists($schema_yolu)) {
            $this->error = "schema.sql bulunamadı: $schema_yolu";
            return;
        }

        $sql = file_get_contents($schema_yolu);
        if ($sql === false) {
            $this->error = "schema.sql okunamadı";
            return;
        }

        $ok = $this->db->exec($sql);
        if (!$ok) {
            $this->error = "schema.sql exec hatası: " . $this->db->lastErrorMsg();
        }
    }
